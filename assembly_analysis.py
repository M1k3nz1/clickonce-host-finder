...
class AssemblyReport:
    ...
    manifest_top_assembly_identity: bool = False
    manifest_top_identity_commented: bool = False
    manifest_dependency_identities: int = 0
...
def is_managed_executable(exe_path: Path) -> bool:
    """Fast PE check for a CLR (.NET) assembly — avoids spawning PowerShell on native EXEs."""
    pe = None
    try:
...
def pe_quick_scan(exe_path: Path) -> PeQuickScan:
    """Single PE pass for cheap pre-filters before PowerShell analysis."""
    pe = None
    result = PeQuickScan()
    try:
        pe = pefile.PE(str(exe_path), fast_load=True)
        ...
        if cor_flags is None:
            return result
        result.managed = True
        result.mixed_mode = not bool(cor_flags & COMIMAGE_FLAGS_ILONLY)

        machine = pe.FILE_HEADER.Machine
        arch_map = {0x8664: "amd64", 0x200: "ia64", 0x1c0: "arm", 0xaa64: "arm64"}
        if machine in arch_map:
            result.architecture = arch_map[machine]
        elif machine == 0x14c:
            if cor_flags & (COMIMAGE_FLAGS_32BITREQUIRED | COMIMAGE_FLAGS_32BITPREFERRED):
                result.architecture = "x86"
            else:
                result.architecture = "msil"
        else:
            result.architecture = "msil"
...
def architecture_matches_filter(architecture: str, arch_filter: str) -> bool:
    if arch_filter == "any" or not architecture:
        return True
    normalized = architecture.lower()
    if arch_filter == "x64":
        return normalized in {"amd64", "x64"}
    if arch_filter == "x86":
        return normalized == "x86"
    return normalized == "msil"
...
def _is_assembly_identity_tag(tag: str) -> bool:
    return tag == f"{ASM_NS}assemblyIdentity" or tag.endswith("assemblyIdentity")


def _assembly_identity_in_xml_comments(manifest_xml: str) -> bool:
    for match in re.finditer(r"<!--(.*?)-->", manifest_xml, flags=re.DOTALL):
        if "assemblyIdentity" in match.group(1):
            return True
    return False


def analyze_manifest_assembly_identity(
    manifest_xml: Optional[str],
) -> tuple[bool, bool, int, str, bool]:
    """
    Inspect the embedded Win32 application manifest (not .NET CLI metadata).

    Returns:
        top_level_active, top_level_commented_only, dependency_count,
        top_processor_arch, wildcard_on_top_level
    """
    if not manifest_xml:
    ...
    try:
        ...
        for child in root_children:
            if _is_assembly_identity_tag(child.tag):
                top_level_active = True
                top_processor_arch = child.attrib.get("processorArchitecture", "")
                wildcard_on_top = top_processor_arch == "*"
                break

        for node in root.iter():
            if not _is_assembly_identity_tag(node.tag):
                continue
        ...
        match = re.search(
            r"<assemblyIdentity\b[^>]*processorArchitecture=[\"']([^\"']+)[\"']",
            manifest_xml,
            flags=re.IGNORECASE,
        )
        if match:
            top_level_active = True
            top_processor_arch = match.group(1)
            wildcard_on_top = top_processor_arch == "*"

    identity_in_comments = _assembly_identity_in_xml_comments(manifest_xml)
    top_level_commented_only = identity_in_comments and not top_level_active
    return (
        top_level_active,
        top_level_commented_only,
        dependency_count,
        top_processor_arch,
        wildcard_on_top,
    )
...
def _parse_manifest_identity(manifest_xml: Optional[str]) -> tuple[str, bool]:
    ...
    (
        top_level_active,
        _top_level_commented_only,
        _dependency_count,
        processor_arch,
        wildcard,
    ) = analyze_manifest_assembly_identity(manifest_xml)
    if top_level_active and processor_arch:
...
def analyze_assembly(exe_path: Path) -> Optional[AssemblyReport]:
    ...
    if not is_managed_executable(exe_path):
        return None
    ...
    (
        manifest_top_assembly_identity,
        manifest_top_identity_commented,
        manifest_dependency_identities,
        manifest_processor_arch,
        manifest_arch_wildcard,
    ) = analyze_manifest_assembly_identity(manifest_xml)
    ...
    return AssemblyReport(
        ...
        manifest_arch_wildcard=manifest_arch_wildcard,
        manifest_top_assembly_identity=manifest_top_assembly_identity,
        manifest_top_identity_commented=manifest_top_identity_commented,
        manifest_dependency_identities=manifest_dependency_identities,
        signed_manifest_profile=manifest_profile,
