...
from assembly_analysis import AssemblyReport, analyze_assembly
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Directory or file to scan (repeatable). Defaults to common install roots.",
    )
    parser.add_argument(
        "--services",
        action="store_true",
...
def native_scan(search_roots: list[Path], *, all_paths: bool) -> list[Path]:
    discovered: list[Path] = []
    seen: set[str] = set()
...
def arch_matches(report: AssemblyReport, arch_filter: str) -> bool:
    if arch_filter == "any":
        return True
    normalized = report.architecture.lower()
    if arch_filter == "x64":
...
def render_table(reports: list[AssemblyReport]) -> str:
    headers = ["status", "arch", "signed", "strong", "mixed", "sig_profile", "name", "path", "notes"]
    rows = [
        [
...
def main() -> None:
    ...
    if args.discovery_tool:
        if not args.discovery_tool.exists():
            print(f"error: discovery tool not found at {args.discovery_tool}", file=sys.stderr)
            sys.exit(1)
        print(f"Using discovery tool: {args.discovery_tool}")
        candidate_paths.extend(
            discovery_tool_scans(
                args.discovery_tool,
                search_roots,
        ...
        print("Using built-in directory walk (pass --discovery-tool for extended enumeration)")

    candidate_paths.extend(native_scan(search_roots, all_paths=args.all_paths))
    ...
    for path in unique_candidates:
        ...
        if report is None or not passes_filters(report, args):
            continue
